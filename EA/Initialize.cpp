...
void Capture_Params(int argc, char** argv){
  ...
  genSize = atoi(argv[5]);
  elitismQ = atoi(argv[6]);
  greedyRandomList = atoi(argv[7]);
  kmoves = atoi(argv[8]);
  pIS = atof(argv[9]);
  pES = atof(argv[10]);
  pAdd = atof(argv[11]);
  ...
  if(argc > 13){
    sint = true;
    bestKnownValue = atoi(argv[13]);
  }
  // cout << mode << " " << InstanceFile << " " << Seed << " " << maxGeneration << " " << genSize << " " << elitismQ<< " " << greedyRandomList<< " " << kmoves << endl;
  // cout << pIS << " " << pES << " " << pAdd << " " << pR << endl;
}
