...
void GenAl(int maxGeneration, int genSize){
	bool testing = false;
	...
	if(testing){
    	bestInd.PrintSolution(true,true,false);
	...
	for (int i = 0; i < maxGeneration; ++i)
	{
		...
		{
			// Add Node
			gen[j] = AddNode(false, gen[j], 0.4);
		}

		FindBest();
		
		// Explore
		// Remove 1 node per route !!!
		for (int j = 0; j < genSize; ++j)
		{
			
			// Considering lossing a whole route, do it later
			gen[j] = RemoveNode(true, gen[j], 0.25); // Take the first 
		}
	...
	FindBest();
	// gen[0].PrintSolution(true,true,false);
	...
	bestInd.PrintSolutionFromRoutes();
...
void FindBest(){
	...
	// cout << "Get Fev first ?" << endl;
	float bestScore = bestInd.FevChangeCostNMilk(0,0,0); // best individual with no changes in Fev
	// cout << "FINISHED" << endl;
	...
	for (long unsigned int i = 0; i < copy.routes.size(); ++i)
	{
		...
		for (int k = 0, counter = 0; counter < totalNodes &&  k < totalNodes; ++k)
		{
			...
			if(selectedAviableNode < (int)copy.aviableFarms[j].size()){
				// cout << "save node in [" << j << "] inner id " << selectedAviableNode << "/"<< (int)copy.aviableFarms[j].size()<< endl;
		...
		// cout <<  i << " listas las opciones " << options.size() << endl;
		...
		// copy.PrintSolution(true,true,true);
		for (long unsigned int j = 0; j < options.size(); ++j){
			// cout << "see options "<< j << endl;
			// cout << replaceInsertPosition << " options[j] " << options[j] << endl;
			float newCostChange = copy.CostFromAddNode(i, replaceInsertPosition, options[j]);
			float newFev = 0;
			int typeNew = GetNodeType(options[j]);

			// cout << "basics ready ? [" << i << "]" << endl;
			if(copy.routes[i].size() == 0){
				newFev = copy.FevChangeCostNMilkAndType(newCostChange,i, GetNodeMilkQuantity(options[j]), typeNew);
			}
			else{
				newFev = copy.FevChangeCostNMilkAndType(newCostChange,i,GetNodeMilkQuantity(options[j]), typeNew);
			}
			// float gain = GetNodeMilkQuantity(options[j]) * (typeNew == 0? aValue : typeNew == 1? bValue : cValue);
			
			// cout << newFev << " " << bestFev << " current option " << options[j]<< " cChange" << newCostChange << " nt" << GetNodeMilkQuantity(options[j]) << " adjusted gain" << gain << "-" << newCostChange << endl;
			...
			if(newFev > bestFev){
				...
				bestMilkDiff = GetNodeMilkQuantity(options[j]);
			}
		}
		if(foundOption != -1){
			int typeNew = GetNodeType(options[foundOption]);
			int typeroute = copy.milkTypePerRoute[i];
			...
			// Assign milktype
			copy.milkTypePerRoute[i] = typeroute >= typeNew? typeroute : typeNew; // Can degrade the current route
			copy.UpdateRouteCost(i,bestCostDiff);
			copy.UpdateRouteMilk(i,bestMilkDiff);
			
			// Add the milk to the route
			// Add the cost to the route
...
Individual RemoveNode(bool blind, Individual ind, float chancePerRoute){
	...
	for (long unsigned int i = 0; i < copy.routes.size(); ++i)
	{
		...
		for (long unsigned int j = 0; j < copy.routes[i].size() && j < maxMoves; ++j){
			...
			// First improvement, for now just keep it as it is
			if(newFev > oldFev || blind){
				// cout << "remove ???" << newFev << " vs " << oldFev << endl;
...
// primera mejora from random first to n final
// The penalty will not change, so we just care about the 
// real change of traveling cost
...
Individual SwapNodeOutside(Individual ind, float chancePerRoute){
	Individual copy = ind;
	for (long unsigned int i = 0; i < copy.routes.size(); ++i)
	{
		// cout << i << " route check " << endl;
		...
		// cout<< "finished ??? " << i << " " << options.size() << endl;
		// now for each posible solution we need Change in milk and change in cost
		float oldFev = copy.FevChangeCostNMilk(0,0,0);
		// copy.PrintSolution(true,true,true);
		for (long unsigned int j = 0; j < options.size(); ++j){
			...
			//  diference
			int typeNew = GetNodeType(options[j]);
			float newFev = copy.FevChangeCostNMilk(costDiff,i, milkDiff);
			// float newFev = copy.FevChangeCostNMilkAndType(costDiff,i, milkDiff, typeNew);
			// cout << newFev << " " << oldFev << " current option " << options[j]<< endl;

			// For now first Improvement
			if(newFev > oldFev){
				// cout << endl << endl << "from " << oldFev <<  " to " << newFev << endl;
				// copy.PrintSolution(true,true,true);
				// cout << "B.1"<< endl;
				// Add node to aviable
				// int typeroute = copy.milkTypePerRoute[i];
				...
				// Finally, update cost from current and milk
				copy.UpdateRouteCost(i,costDiff);
				copy.UpdateRouteMilk(i,milkDiff);
				// copy.milkTypePerRoute[i] = typeroute >= typeNew? typeroute : typeNew;
				// copy.PrintSolution(true,true,true);
				break;
	...
	for (long unsigned  int i = 0; i < milkPerRoute.size(); ++i)
	{
		...
		if(milkTypePerRoute[i] != -1){
			if((int)i != routeid){
				...
				totalMilkCollected[newType] += milkPerRoute[i] + milkChange ;
			}
		}
		else{
			if((int)i == routeid){
...
void Individual::PrintSolutionFromRoutes(){
    ...
    milkGain = (int)(totalMilkCollected[0] * aValue + totalMilkCollected[1] *bValue + totalMilkCollected[2] *cValue);
